...
The goal of this tutorial is to provide a guide to using the [Universal intrinsics](https://docs.opencv.org/5.x/df/d91/group__core__hal__intrin.html) feature to vectorize your C++ code for a faster runtime.
...
Supported SIMD/VLA technologies are detailed in [Universal intrinsics](https://docs.opencv.org/5.x/df/d91/group__core__hal__intrin.html) .
...
```{doxysnippet} univ_intrin.cpp
:tag: convolution-1D-scalar
:language: cpp
```
...
```{doxysnippet} univ_intrin.cpp
:tag: convolution-1D-scalar-main
:language: cpp
```
