...
(tutorial_oneapi_install_quick_start)=
## Quick start
...
* oneDNN replaces the generic kernels of the [cv::dnn](https://docs.opencv.org/5.x/d6/d0f/group__dnn.html) layer with implementations that exploit AVX2, AVX-512, AMX and VNNI, accelerating convolutions, matmul and network post-processing by up to 3-5× on modern CPUs.
* oneTBB takes over the thread pool, scheduling filters like [cv::resize](https://docs.opencv.org/5.x/da/d54/group__imgproc__transform.html#ga47a974309e9102f5f08231edc7e7529d), [cv::GaussianBlur](https://docs.opencv.org/5.x/d4/d86/group__imgproc__filter.html#gae8bdcd9154ed5ca3cbc1766d960f45c1) or the G-API pipeline across all cores without busy-wait.
...
* All this happens transparently: the source code that uses [cv::Mat](https://docs.opencv.org/5.x/d3/d63/classcv_1_1Mat.html) remains the same, but the linked symbols point to vectorized versions, and the internal dispatcher selects the appropriate vector width at runtime.
