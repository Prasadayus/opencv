...
| OPENCV_OCL4DNN_CONVOLUTION_IGNORE_INPUT_DIMS_4_CHECK | bool | false | workaround for OpenCL backend, see [https://github.com/opencv/opencv/issues/20833](https://github.com/opencv/opencv/issues/20833) |
...
**Note:** extra FFmpeg options should be passed in form `key;value|key;value|key;value`, for example `hwaccel;cuvid|video_codec;h264_cuvid|vsync;0` or `vcodec;x264|vprofile;high|vlevel;4.0`
...
| OPENCV_DSHOW_SAVEGRAPH_FILENAME | file path | | enable processing graph dump in the DShow backend |
...
| OPENCV_HIGHGUI_FB_MODE | string | `FB` | Selects output mode for the framebuffer backend (`FB` - regular framebuffer, `EMU` - emulation, perform internal checks but does nothing, `XVFB` - compatible with _xvfb_ virtual framebuffer) |
| OPENCV_HIGHGUI_FB_DEVICE | file path | | Path to framebuffer device to use (will be checked first) |
