...
suppress_warnings = ["myst.header", "toc.excluded"]

breathe_projects = {
    "OpenCV": os.path.join(os.path.dirname(__file__), "../build/doc/doxygen/xml"),
}
...
def setup(app):
    ...
    # exhale 0.3.x crashes in multiple places when Doxygen XML contains pages with
    # no title (happens with legacy .markdown tutorial files that lack a Doxygen
    # page anchor). Fix at the root: ensure ExhaleNode.title is never None.
    import exhale.graph as _eg_graph
    _orig_node_init = _eg_graph.ExhaleNode.__init__
    def _safe_node_init(self, *args, **kwargs):
        _orig_node_init(self, *args, **kwargs)
        if not getattr(self, "title", None):
            self.title = getattr(self, "name", "") or ""
    _eg_graph.ExhaleNode.__init__ = _safe_node_init
