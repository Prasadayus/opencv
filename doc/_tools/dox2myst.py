...
def transform(text: str, tags: TagIndex, *,
    ...
    def asset_link(m: re.Match) -> str:
        ...
        return f"[{label}]({image_prefix}/{target})"
    text = re.sub(
...
def main() -> int:
    ...
    ap.add_argument("--image-prefix", default=None,
                    help="Image path prefix in output (default: <input_dirname>/images)")
    args = ap.parse_args()
    ...
    image_prefix = args.image_prefix or f"{args.input.parent.name}/images"
